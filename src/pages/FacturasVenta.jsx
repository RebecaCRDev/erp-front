...
export default function FacturasVenta() {
  ...
  const [form, setForm] = useState({ id_cliente: '', id_almacen: '', fecha_factura: new Date().toISOString().split('T')[0], lineas: [{ id_producto: '', cantidad: 1, precio_venta: '' }] });
  const [error, setError] = useState('');
  const [filtros, setFiltros] = useState({ id_cliente: '', fecha_desde: '', fecha_hasta: '' });
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const r = await api.get('/facturas-venta/', { params });
  ...
  const addLine = () => setForm({...form, lineas: [...form.lineas, { id_producto: '', cantidad: 1, precio_venta: '' }]});
  ...
  const calcTotal = () => form.lineas.reduce((sum, l) => sum + (parseFloat(l.cantidad || 0) * parseFloat(l.precio_venta || 0)), 0);

  const handleSave = async () => {
    setError('');
    if (!form.id_cliente || !form.id_almacen) { setError('Selecciona cliente y almacen'); return; }
    if (form.lineas.length === 0) { setError('Agrega al menos una linea'); return; }
    for (const l of form.lineas) { if (!l.id_producto || !l.cantidad || !l.precio_venta) { setError('Completa todos los campos de cada linea'); return; } }
    try {
      await api.post('/facturas-venta/', {
        id_cliente: parseInt(form.id_cliente),
        id_almacen: parseInt(form.id_almacen),
        fecha_factura: form.fecha_factura,
        lineas: form.lineas.map(l => ({ id_producto: parseInt(l.id_producto), cantidad: parseInt(l.cantidad), precio_venta: parseFloat(l.precio_venta) }))
      });
      setShowModal(false); fetchData();
    } catch (err) { setError(err.response?.data?.detail || 'Error'); }
  };

  const handleDelete = async (id) => { if (!confirm('Eliminar factura? El stock se devolvera al almacen.')) return; try { await api.delete('/facturas-venta/' + id); fetchData(); } catch (err) { alert(err.response?.data?.detail || 'Error'); } };

  const openDetalle = async (id) => {
    try { const r = await api.get('/facturas-venta/' + id); setDetalleFactura(r.data); setShowDetalle(true); }
    catch (err) { alert('Error cargando detalle'); }
  };

  const limpiarFiltros = () => { setFiltros({ id_cliente: '', fecha_desde: '', fecha_hasta: '' }); setPage(1); };
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
        <h1 style={{ fontSize: '28px', fontWeight: '600', color: '#333' }}>Facturas de Venta</h1>
        <button onClick={() => { setForm({ id_cliente: '', id_almacen: '', fecha_factura: new Date().toISOString().split('T')[0], lineas: [{ id_producto: '', cantidad: 1, precio_venta: '' }] }); setError(''); setShowModal(true); }} style={{ padding: '10px 24px', background: 'linear-gradient(135deg, #2e7d6f, #3a9d8f)', color: 'white', border: 'none', borderRadius: '10px', cursor: 'pointer', fontSize: '14px', fontWeight: '600' }}>+ Nueva Factura</button>
      </div>
      ...
      <div style={{ background: 'white', borderRadius: '12px', padding: '16px 20px', marginBottom: '16px', boxShadow: '0 2px 8px rgba(0,0,0,0.04)', display: 'flex', gap: '12px', alignItems: 'end', flexWrap: 'wrap' }}>
        <div style={{ flex: '1', minWidth: '150px' }}><label style={{ fontSize: '12px', color: '#888', display: 'block', marginBottom: '4px' }}>Cliente</label><select style={inputStyle} value={filtros.id_cliente} onChange={e => { setFiltros({...filtros, id_cliente: e.target.value}); setPage(1); }}><option value="">Todos</option>{clientes.map(c => <option key={c.id_cliente} value={c.id_cliente}>{c.nombre}</option>)}</select></div>
      ...
      <div style={{ background: 'white', borderRadius: '16px', boxShadow: '0 2px 12px rgba(0,0,0,0.04)', overflow: 'hidden' }}>
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          <thead><tr style={{ background: 'linear-gradient(135deg, #2e7d6f, #3a9d8f)' }}>
            {['ID', 'Cliente', 'Almacen', 'Fecha', 'Total', 'Acciones'].map(h => <th key={h} style={{ padding: '14px 16px', color: 'white', textAlign: 'left', fontSize: '13px', fontWeight: '600' }}>{h}</th>)}
          </tr></thead>
          <tbody>
            {loading ? <tr><td colSpan={6} style={{ padding: '40px', textAlign: 'center', color: '#999' }}>Cargando...</td></tr>
            : items.length === 0 ? <tr><td colSpan={6} style={{ padding: '40px', textAlign: 'center', color: '#999' }}>No hay facturas</td></tr>
            : items.map((f, i) => (
              <tr key={f.id_factura_venta} style={{ background: i % 2 === 0 ? '#fafafa' : 'white' }}>
                <td style={{ padding: '12px 16px', fontSize: '14px' }}>
                  <span onClick={() => openDetalle(f.id_factura_venta)} style={{ color: '#2e7d6f', cursor: 'pointer', fontWeight: '600', textDecoration: 'underline' }}>#{f.id_factura_venta}</span>
                </td>
                <td style={{ padding: '12px 16px', fontSize: '14px', fontWeight: '500' }}>{f.cliente_nombre}</td>
                <td style={{ padding: '12px 16px', fontSize: '14px' }}>{f.almacen_nombre}</td>
                <td style={{ padding: '12px 16px', fontSize: '14px' }}>{f.fecha_factura}</td>
                <td style={{ padding: '12px 16px', fontSize: '14px', fontWeight: '600', color: '#2e7d6f' }}>{f.total.toFixed(2)} EUR</td>
                <td style={{ padding: '12px 16px' }}>
                  <div style={{ display: 'flex', gap: '8px' }}>
                    <button onClick={() => openDetalle(f.id_factura_venta)} style={{ padding: '6px 12px', background: '#f3e5f5', color: '#7b1fa2', border: 'none', borderRadius: '6px', cursor: 'pointer', fontSize: '13px', fontWeight: '600' }}>Ver</button>
                    {isAdmin && <button onClick={() => handleDelete(f.id_factura_venta)} style={{ padding: '6px 12px', background: '#ffebee', color: '#c62828', border: 'none', borderRadius: '6px', cursor: 'pointer', fontSize: '13px' }}>Eliminar</button>}
                  </div>
                </td>
              </tr>
            ))}
        ...
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }} onClick={() => setShowDetalle(false)}>
          <div style={{ background: 'white', borderRadius: '16px', width: '700px', maxHeight: '90vh', overflowY: 'auto' }} onClick={e => e.stopPropagation()}>
            <div style={{ background: 'linear-gradient(135deg, #2e7d6f, #3a9d8f)', padding: '24px 32px', borderRadius: '16px 16px 0 0', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div>
                <div style={{ color: 'white', fontSize: '22px', fontWeight: '700' }}>FACTURA VENTA</div>
                <div style={{ color: 'rgba(255,255,255,0.8)', fontSize: '14px' }}>FV-{String(detalleFactura.id_factura_venta).padStart(4, '0')}</div>
              </div>
              <div style={{ textAlign: 'right' }}>
                <div style={{ color: 'white', fontSize: '16px', fontWeight: '600' }}>{detalleFactura.cliente_nombre}</div>
              </div>
            </div>
            <div style={{ padding: '24px 32px' }}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '16px', marginBottom: '24px' }}>
                <div><div style={{ fontSize: '11px', color: '#999', textTransform: 'uppercase', fontWeight: '600', marginBottom: '4px' }}>Cliente</div><div style={{ fontSize: '15px', fontWeight: '600', color: '#333' }}>{detalleFactura.cliente_nombre}</div></div>
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', marginBottom: '20px' }}>
                <thead><tr style={{ borderBottom: '2px solid #2e7d6f' }}>
                ...
                <tbody>{detalleFactura.lineas.map(l => (
                  <tr key={l.id_linea} style={{ borderBottom: '1px solid #f0f0f0' }}>
                    ...
                    <td style={{ padding: '10px 0', fontSize: '14px', textAlign: 'right' }}>{l.precio_venta.toFixed(2)} EUR</td>
                    <td style={{ padding: '10px 0', fontSize: '14px', textAlign: 'right', fontWeight: '600' }}>{l.importe.toFixed(2)} EUR</td>
                  </tr>
                ))}</tbody>
              </table>
              <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
                <div style={{ padding: '16px 20px', background: '#f0faf7', borderRadius: '12px', border: '2px solid #2e7d6f' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', gap: '40px', fontSize: '20px', fontWeight: '700', color: '#2e7d6f' }}><span>TOTAL</span><span>{detalleFactura.total.toFixed(2)} EUR</span></div>
                </div>
              </div>
              <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: '20px' }}>
                <button onClick={() => setShowDetalle(false)} style={{ padding: '10px 24px', background: 'linear-gradient(135deg, #2e7d6f, #3a9d8f)', color: 'white', border: 'none', borderRadius: '10px', cursor: 'pointer', fontWeight: '600' }}>Cerrar</button>
              </div>
            </div>
          </div>
        </div>
      ...
      {showModal && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }} onClick={() => setShowModal(false)}>
          <div style={{ background: 'white', borderRadius: '20px', padding: '32px', width: '650px', maxHeight: '90vh', overflowY: 'auto' }} onClick={e => e.stopPropagation()}>
            <h2 style={{ fontSize: '22px', fontWeight: '600', marginBottom: '24px' }}>Nueva Factura de Venta</h2>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
              ...
              <h3 style={{ fontSize: '16px', color: '#2e7d6f', marginTop: '8px' }}>Lineas de factura</h3>
              {form.lineas.map((l, i) => (
                <div key={i} style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr auto', gap: '8px', alignItems: 'end' }}>
                  <div><label style={{ fontSize: '12px', color: '#666' }}>Producto</label><select style={inputStyle} value={l.id_producto} onChange={e => updateLine(i, 'id_producto', e.target.value)}><option value="">Seleccionar...</option>{productos.map(p => <option key={p.id_producto} value={p.id_producto}>{p.nombre} - {p.talla} ({p.precio_venta} EUR)</option>)}</select></div>
                  ...
                  <div><label style={{ fontSize: '12px', color: '#666' }}>Precio</label><input style={inputStyle} type="number" step="0.01" value={l.precio_venta} onChange={e => updateLine(i, 'precio_venta', e.target.value)} /></div>
              ...
              <div style={{ textAlign: 'right', fontSize: '18px', fontWeight: '700', color: '#2e7d6f', padding: '12px 0', borderTop: '2px solid #f0f0f0' }}>Total: {calcTotal().toFixed(2)} EUR</div>
            </div>
            <div style={{ display: 'flex', gap: '12px', justifyContent: 'flex-end', marginTop: '20px' }}>
              ...
              <button onClick={handleSave} style={{ padding: '10px 24px', background: 'linear-gradient(135deg, #2e7d6f, #3a9d8f)', color: 'white', border: 'none', borderRadius: '10px', cursor: 'pointer', fontWeight: '600' }}>Crear Factura</button>
            </div>
          </div>
        </div>
      )}
