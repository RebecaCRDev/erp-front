...
const initialForm = { id_proveedor: '', nombre: '', tipo_prenda: 'manga_corta', talla: 'M', color: '', precio_compra: '', descripcion: '', estado: 'activo' };
...
export default function ProductosProveedor() {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const res = await api.get('/productos-proveedor/', { params: { page, per_page: 10, search } });
      setItems(res.data.data); setTotalPages(res.data.total_pages); setTotal(res.data.total);
    } catch {} finally { setLoading(false); }
  };

  useEffect(() => { fetchData(); }, [page, search]);
  useEffect(() => { api.get('/proveedores/', { params: { per_page: 100 } }).then(r => setProveedores(r.data.data)).catch(() => {}); }, []);

  const openCreate = () => { setEditing(null); setForm(initialForm); setError(''); setShowModal(true); };
  const openEdit = (item) => {
    setEditing(item.id_producto_proveedor);
    setForm({ id_proveedor: item.id_proveedor, nombre: item.nombre, tipo_prenda: item.tipo_prenda, talla: item.talla, color: item.color || '', precio_compra: item.precio_compra, descripcion: item.descripcion || '', estado: item.estado });
    setError(''); setShowModal(true);
  };

  const handleSave = async () => {
    setError('');
    if (!form.nombre.trim() || !form.id_proveedor || !form.precio_compra) { setError('Nombre, proveedor y precio son obligatorios'); return; }
    const payload = { ...form, precio_compra: parseFloat(form.precio_compra), id_proveedor: parseInt(form.id_proveedor) };
    try {
      if (editing) { await api.put('/productos-proveedor/' + editing, payload); }
      else { await api.post('/productos-proveedor/', payload); }
  ...
  const handleDelete = async (id) => {
    ...
    try { await api.delete('/productos-proveedor/' + id); fetchData(); } catch (err) { alert(err.response?.data?.detail || 'Error'); }
  };
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
        <h1 style={{ fontSize: '28px', fontWeight: '600', color: '#333' }}>Productos Proveedor</h1>
      ...
      <div style={{ background: 'white', borderRadius: '16px', boxShadow: '0 2px 12px rgba(0,0,0,0.04)', overflow: 'hidden' }}>
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          <thead><tr style={{ background: 'linear-gradient(135deg, #2e7d6f, #3a9d8f)' }}>
            {['ID', 'Nombre', 'Tipo', 'Talla', 'Color', 'Precio', 'Proveedor', 'Estado', 'Acciones'].map(h => <th key={h} style={{ padding: '14px 16px', color: 'white', textAlign: 'left', fontSize: '13px', fontWeight: '600' }}>{h}</th>)}
          </tr></thead>
          <tbody>
            ...
            : items.map((c, i) => (
              <tr key={c.id_producto_proveedor} style={{ background: i % 2 === 0 ? '#fafafa' : 'white', borderBottom: '1px solid #f0f0f0' }}>
                <td style={{ padding: '12px 16px', fontSize: '14px' }}>{c.id_producto_proveedor}</td>
                ...
                <td style={{ padding: '12px 16px', fontSize: '14px', fontWeight: '600' }}>{c.precio_compra} EUR</td>
                <td style={{ padding: '12px 16px', fontSize: '14px' }}>{c.proveedor?.nombre || '-'}</td>
                ...
                <td style={{ padding: '12px 16px' }}>
                  <div style={{ display: 'flex', gap: '8px' }}>
                    ...
                    {isAdmin && <button onClick={() => handleDelete(c.id_producto_proveedor)} style={{ padding: '6px 12px', background: '#ffebee', color: '#c62828', border: 'none', borderRadius: '6px', cursor: 'pointer', fontSize: '13px' }}>Eliminar</button>}
                  </div>
                </td>
              </tr>
            ))}
      ...
      {showModal && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }} onClick={() => setShowModal(false)}>
          <div style={{ background: 'white', borderRadius: '20px', padding: '32px', width: '500px', maxHeight: '90vh', overflowY: 'auto' }} onClick={(e) => e.stopPropagation()}>
            <h2 style={{ fontSize: '22px', fontWeight: '600', color: '#333', marginBottom: '24px' }}>{editing ? 'Editar' : 'Nuevo'} Producto Proveedor</h2>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                ...
                <div><label style={{ fontSize: '13px', color: '#666', marginBottom: '4px', display: 'block' }}>Precio compra *</label><input style={inputStyle} type="number" step="0.01" value={form.precio_compra} onChange={(e) => setForm({...form, precio_compra: e.target.value})} /></div>
              </div>
