...
export default function Clientes() {
  ...
  const fetchClientes = async () => {
    setLoading(true);
    try {
      const res = await api.get('/clientes/', { params: { page, per_page: 10, search } });
  ...
  const fetchProvincias = async () => {
    try {
      const res = await api.get('/clientes/provincias/todas');
  ...
  const openEdit = (cliente) => {
    ...
    setForm({
      nombre: cliente.nombre || '', email: cliente.email || '', telefono: cliente.telefono || '', estado: cliente.estado || 'activo',
      direccion: cliente.direccion ? { calle: cliente.direccion.calle || '', numero: cliente.direccion.numero || '', codigo_postal: cliente.direccion.codigo_postal || '', ciudad: cliente.direccion.ciudad || '', id_provincia: cliente.direccion.id_provincia || 1 } : { calle: '', numero: '', codigo_postal: '', ciudad: '', id_provincia: 1 },
    });
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
        <h1 style={{ fontSize: '28px', fontWeight: '600', color: '#333' }}>Clientes</h1>
        {(isAdmin || canEditClientes) && <button onClick={openCreate} style={{ padding: '10px 24px', background: 'linear-gradient(135deg, #2e7d6f, #3a9d8f)', color: 'white', border: 'none', borderRadius: '10px', cursor: 'pointer', fontSize: '14px', fontWeight: '600' }}>+ Nuevo Cliente</button>}
      </div>
      <div style={{ marginBottom: '20px' }}>
        <input type="text" placeholder="Buscar por nombre, email o telefono..." value={search} onChange={(e) => { setSearch(e.target.value); setPage(1); }} style={{ ...inputStyle, maxWidth: '400px' }} />
      </div>
      <div style={{ background: 'white', borderRadius: '16px', boxShadow: '0 2px 12px rgba(0,0,0,0.04)', overflow: 'hidden' }}>
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          <thead><tr style={{ background: 'linear-gradient(135deg, #2e7d6f, #3a9d8f)' }}>
            {['ID', 'Nombre', 'Email', 'Telefono', 'Estado', (isAdmin || canEditClientes) ? 'Acciones' : ''].filter(Boolean).map((h) => (
          ...
          <tbody>
            {loading ? (
              ...
              <tr><td colSpan={6} style={{ padding: '40px', textAlign: 'center', color: '#999' }}>No hay clientes</td></tr>
            ) : clientes.map((c, i) => (
              <tr key={c.id_cliente} style={{ background: i % 2 === 0 ? '#fafafa' : 'white', borderBottom: '1px solid #f0f0f0' }}>
                <td style={{ padding: '12px 16px', fontSize: '14px' }}>{c.id_cliente}</td>
                ...
                {(isAdmin || canEditClientes) && (
                  <td style={{ padding: '12px 16px' }}>
                    <div style={{ display: 'flex', gap: '8px' }}>
                      ...
                      {isAdmin && <button onClick={() => handleDelete(c.id_cliente)} style={{ padding: '6px 12px', background: '#ffebee', color: '#c62828', border: 'none', borderRadius: '6px', cursor: 'pointer', fontSize: '13px' }}>Eliminar</button>}
                    </div>
                  </td>
                )}
              </tr>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '16px', borderTop: '1px solid #f0f0f0' }}>
          <span style={{ fontSize: '13px', color: '#999' }}>{total} clientes en total</span>
          <div style={{ display: 'flex', gap: '8px' }}>
      ...
      {showModal && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }} onClick={() => setShowModal(false)}>
          <div style={{ background: 'white', borderRadius: '20px', padding: '32px', width: '500px', maxHeight: '90vh', overflowY: 'auto' }} onClick={(e) => e.stopPropagation()}>
            <h2 style={{ fontSize: '22px', fontWeight: '600', color: '#333', marginBottom: '24px' }}>{editing ? 'Editar Cliente' : 'Nuevo Cliente'}</h2>
            ...
            <div style={{ display: 'flex', gap: '12px', justifyContent: 'flex-end', marginTop: '28px' }}>
              ...
              <button onClick={handleSave} style={{ padding: '10px 24px', background: 'linear-gradient(135deg, #2e7d6f, #3a9d8f)', color: 'white', border: 'none', borderRadius: '10px', cursor: 'pointer', fontSize: '14px', fontWeight: '600' }}>{editing ? 'Guardar cambios' : 'Crear cliente'}</button>
            </div>
          </div>
        </div>
      )}
